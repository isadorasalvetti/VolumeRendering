...
bool RawReader::readMesh(const QString &filename, TriangleMesh &mesh)
{
	ifstream fin;
...
bool RawReader::loadHeader(ifstream &fin, int &nVertices, int &nFaces)
{
	char line[100];
...
void RawReader::loadVertices(ifstream &fin, int nVertices, vector<float> &plyVertices)
{
	int i;
	float value;
...
void RawReader::loadFaces(ifstream &fin, int nFaces, vector<int> &plyTriangles)
{
	int i, tri[3];
...
void RawReader::rescaleModel(vector<float> &plyVertices)
{
	unsigned int i;
...
void RawReader::addModelToMesh(const vector<float> &plyVertices, const vector<int> &plyTriangles, TriangleMesh &mesh)
{
	unsigned int i;
